...
contract('InitIssueAndLockFactory', (accounts) =>{
  ...
  context('init', ()=>{
    it("init", async() =>{
      ...
      await gttoken.add_trusted_issuer(invoke_id, init.address, {from:accounts[1]});
      await gttoken.add_trusted_issuer(invoke_id, init.address, {from:accounts[0]});
  })
  })

  context('init_lock', ()=>{
      it('replace', async ()=>{
      invoke_id = await gttoken.get_unused_invoke_id("replace", {from:accounts[0]});
