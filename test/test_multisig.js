...
contract("TestMultiSigUpgrade", (accounts) =>{
  ...
  context('init', ()=>{
    it("init", async() =>{
          multisig_factory = await MultiSigFactory.deployed();
          assert.ok(multisig_factory);
          oldmultisig_factory = await OldMultiSigFactory.deployed();
          assert.ok(oldmultisig_factory);

          tokentx = await multisig_factory.createMultiSig(accounts.slice(5, 9));
          multisig = await MultiSig.at(tokentx.logs[0].args.addr);
          assert.ok(multisig);

          tokentx = await oldmultisig_factory.createMultiSig(accounts.slice(0, 3));
          oldmultisig = await OldMultiSig.at(tokentx.logs[0].args.addr);
